...
class VideoControlsPlaybackExtrasLoader {
  ...
  Future<PlaybackExtras?> load({bool forceRefresh = false}) async {
    ...
    // Aşama 2: Dosya içinde jenerik bulunamadıysa TheIntroDB API sorgulanır
    if (!fileHasIntro || !fileHasCredits) {
      try {
        ...
        if (introDbMarkers.isNotEmpty) {
          ...
          appLogger.i('Aşama 2 (TheIntroDB): Merged ${introDbMarkers.length} markers for ${metadata.displayTitle}');
        }
      } catch (e) {
        appLogger.w('Failed to merge TheIntroDB markers', error: e);
      }
    } else {
      appLogger.i('Aşama 1 (Dosya Bölümleri): Jenerik dosya içi bölümlerden tespit edildi, API çağrısı atlanıyor.');
    }

    // Aşama 3: Hem dosya içi bölümlerde hem de TheIntroDB API'sinde jenerik yoksa Seçenek A devreye girer
    ...
    if (!hasIntroAfterApi && metadata.kind == MediaKind.episode) {
      ...
      mergedMarkers.add(MediaMarker(
        id: 9999,
        type: 'intro',
        startTimeOffset: 60000,   // 01:00 (60,000 ms)
        endTimeOffset: 150000,   // 02:30 (150,000 ms)
      ));
      ...
      appLogger.i('Aşama 3 (Seçenek A - İstemci İçi Akıllı İntro): Injected fallback intro marker (01:00 - 02:30) for ${metadata.displayTitle}');
    }
